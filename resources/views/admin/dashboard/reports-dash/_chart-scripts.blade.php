...
<script>
    ...
    const ordersByMonth = {!! json_encode($ordersChart ?? []) !!};
    const revenueByMonth = {!! json_encode($revenueChart ?? []) !!};
    const userTypes = {!! json_encode($userTypes ?? ['Regular', 'Admin']) !!};
    const userTypeCounts = {!! json_encode($userTypeCounts ?? [0, 0]) !!};
    const reportsStatusData = {!! json_encode($reportsStatusData ?? [0, 0]) !!};
    const reportsStatusLabels = {!! json_encode($reportsStatusLabels ?? ['Open', 'Resolved']) !!};
