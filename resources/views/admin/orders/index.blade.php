                ...
                <tbody class="bg-white dark:bg-[#23263a] divide-y divide-gray-200 dark:divide-[#23263a]">
                    @forelse($orders as $order)
                        <tr class="hover:bg-gray-300 dark:hover:bg-[#262c47]">
                            ...
                            <td class="px-6 py-4 text-indigo-700 dark:text-indigo-300 ">${{ number_format($order->total_price, 2) }}</td>
                            <td class="px-6 py-4">
                                <span class="inline-block px-2 py-1 rounded-full text-xs  {{ $order->payment_status == 'paid' ? 'bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300' : 'bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-300' }}">
