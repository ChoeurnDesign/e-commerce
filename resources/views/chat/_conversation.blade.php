...
@if (isset($currentChat) && $currentChat)
    <div x-data="chatComponent()" x-init="initChat({{ $currentChat->id }}, {{ json_encode($lastSentId ?? null) }}, {{ json_encode($lastSentMessageTs ?? null) }})" class="flex flex-col h-full">
        @php
        ...
        <div class="flex-shrink-0 px-4 py-3.5 bg-gray-100 dark:bg-gray-800 flex items-center justify-between border-b border-gray-300 dark:border-gray-700">
            <div class="flex items-center flex-1">
                ...
                <div class="h-10 w-10 rounded-full bg-gray-300 dark:bg-gray-500 flex items-center justify-center overflow-hidden">
                    @if ($chatAvatar)
                        <img src="{{ $chatAvatar }}" alt="{{ $chatName }}" class="h-full w-full object-cover">
                    @else
                        @php
                ...
                <div class="ml-3 flex-1">
                    <h3 class="text-base font-medium text-gray-900 dark:text-white">{{ $chatName }}</h3>
                    <div class="flex items-center">
                        <span class="text-xs {{ $isOnline ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400' }}">
                            @if ($otherUser)
                                {{ $otherUser->getChatActivityStatus($userTimezone) }}
                            @else
                                Last seen recently
                            @endif
                        </span>
    ...
    @foreach ($messages as $index => $message)
        ...
        @if ($currentDate !== $messageDate)
            ...
            <div class="flex justify-center my-4">
                <span class="px-3 py-1 text-xs text-gray-600 dark:text-gray-300 bg-gray-200 dark:bg-gray-600 rounded-full">
                    @if ($messageInUserTimezone->isToday())
                        Today
                    @elseif($messageInUserTimezone->isYesterday())
                        Yesterday
                    @else
                        {{ $messageInUserTimezone->format('M j, Y') }}
                    @endif
                </span>
            </div>
        @endif
        ...
        @if ($showGroupTime)
            <div class="flex justify-center my-2">
                <span class="text-xs text-gray-500 dark:text-gray-400">
                    {{ strtoupper($messageInUserTimezone->format('g:i A')) }}
                </span>
            </div>
        @endif 
        ...
        <div class="flex {{ $message->sender_id === auth()->id() ? 'justify-end' : 'justify-start' }}"
            ...
            data-group-time="{{ $showGroupTime ? 1 : 0 }}">
            ...
            <div class="relative w-full">
                ...
                @unless ($showGroupTime)
                    <div x-show="showMessageTime === {{ $message->id }}"
                ...
                <div class="flex items-start w-full {{ $message->sender_id === auth()->id() ? 'justify-end' : '' }}">
                    @if ($message->sender_id !== auth()->id())
                        <div class="flex-shrink-0 mr-2">
                            <div class="h-8 w-8 rounded-full bg-gray-300 dark:bg-gray-600 flex items-center justify-center overflow-hidden">
                                @if ($senderAvatar)
                                    <img src="{{ $senderAvatar }}" alt="{{ $message->sender ? $message->sender->name : 'Unknown' }}" class="h-full w-full object-cover">
                                @else
                                    @php
                    ...
                    {{-- bubble area --}}
                    <div class="w-full flex flex-col {{ $message->sender_id === auth()->id() ? 'items-end' : 'items-start' }}" style="max-width:75%;">
                        <div
                            @click.stop.prevent="toggleMessage({{ $message->id }}, {{ $showGroupTime ? 'true' : 'false' }})"
                            x-bind:class="{
                                'shadow-sm cursor-pointer hover:shadow-md transition-all px-3 py-2 relative z-10 select-none': true,
                                'mt-4 message-shift': (showMessageTime === {{ $message->id }} || showMessageStatus === {{ $message->id }})
                            }"
                            @class([
                        ...
                        @if ($message->sender_id === auth()->id())
                            <div class="mt-1 text-sm relative flex items-center space-x-1">
                                ...
                                @endphp
                                <div
                                    class="text-xs relative message-status pointer-events-none {{ $message->is_read ? 'text-green-500 dark:text-green-400' : 'text-gray-500 dark:text-gray-400' }}"
                                    data-message-id="{{ $message->id }}"
                                    data-is-read="{{ $message->is_read ? 1 : 0 }}"
                                    x-show="showMessageStatus === {{ $message->id }} || showMessageTime === {{ $message->id }} || {{ $showByDefault ? 'true' : 'false' }}"
