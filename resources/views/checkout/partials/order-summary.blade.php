    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 sticky top-4 border-2 border-gray-300 dark:border-gray-700">
            ...
            @foreach($cartItems as $item)
                <div class="flex items-center space-x-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-sm font-medium text-gray-900 dark:text-gray-100">${{ number_format($item['subtotal'], 2) }}</p>
                    </div>
                </div>
            @endforeach
        </div>
        ...
        <div class="border-t border-gray-200 dark:border-gray-700 pt-4 space-y-3">
            <div class="flex justify-between text-sm">
                <span class="text-gray-600 dark:text-gray-300">Subtotal ({{ $cartTotals['total_quantity'] }} items)</span>
                <span class="font-medium">${{ number_format($cartTotals['subtotal'], 2) }}</span>
            </div>
            <div class="flex justify-between text-sm">
                <span class="text-gray-600 dark:text-gray-300">Tax ({{ number_format($cartTotals['tax_rate'] * 100, 1) }}%)</span>
                <span class="font-medium">${{ number_format($cartTotals['tax'], 2) }}</span>
            </div>
            <div class="flex justify-between text-sm">
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 pt-3">
                <div class="flex justify-between text-lg font-bold">
                    ...
                    <span class="text-gray-900 dark:text-gray-100">${{ number_format($cartTotals['total'], 2) }}</span>
                </div>
            </div>
        </div>
        <!-- Place Order Button -->
