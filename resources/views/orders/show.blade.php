        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6 mb-6">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Order #{{ $order->order_number }}</h1>
                        <p class="text-gray-600 dark:text-gray-400 mt-2">Placed on {{ $order->created_at->format('F d, Y \a\t h:i A') }}</p>
                    </div>
                    <div class="mt-4 md:mt-0">
                        ...
                        @endphp
                        <span class="inline-flex px-4 py-2 text-sm font-semibold rounded-lg border {{ $statusColors[$order->status] ?? 'bg-gray-300 text-gray-800 border-gray-200 dark:bg-gray-700 dark:text-gray-100 dark:border-gray-600' }}">
                ...
                <div class="lg:col-span-2">
                    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                            <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Order Items ({{ $order->orderItems->count() }} items)</h2>
                        </div>
                            ...
                            <div class="space-y-2">
                                <div class="flex justify-between text-sm">
                                    ...
                                    <span class="text-gray-900 dark:text-gray-100">
                                        {{ \App\Helpers\CurrencyHelper::format($order->subtotal) }}
                                    </span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-600 dark:text-gray-400">Tax</span>
                                    <span class="text-gray-900 dark:text-gray-100">
                                        {{ \App\Helpers\CurrencyHelper::format($order->tax_amount) }}
                                    </span>
                                </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
                        ...
                        <div class="space-y-3">
                            <div>
                                ...
                                <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ ucfirst(str_replace('_', ' ', $order->payment_method)) }}</p>
                            </div>
                            <div>
                                <span class="text-sm text-gray-600 dark:text-gray-400">Payment Status</span>
                                <p class="text-sm font-medium {{ $order->payment_status === 'paid' ? 'text-green-600 dark:text-green-400' : 'text-yellow-600 dark:text-yellow-400' }}">
                            ...
                            @if($order->payment_id)
                                <div>
                                    ...
                                    <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $order->payment_id }}</p>
                                </div>
                            @endif
                        </div>
                    </div>
