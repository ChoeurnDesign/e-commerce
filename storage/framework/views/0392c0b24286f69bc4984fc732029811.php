...
<div id="social-links-wrapper" class="space-y-4" data-initial='<?php echo json_encode($initialSocials, 15, 512) ?>'>
        ...
        <div id="social-platforms" class="grid grid-cols-4 gap-3 mb-4">
            <?php $__currentLoopData = $platforms; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <button type="button"
                    ...
                    data-key="<?php echo e($p); ?>"
                    aria-pressed="false"
                    title="<?php echo e(ucfirst($p)); ?>"
                >
                    <span class="flex items-center justify-center w-9 h-9 rounded-full <?php echo e($bgClasses[$p] ?? 'bg-gray-100 dark:bg-gray-600'); ?>">
                        <?php echo view('components.icon-socials', ['name' => $p, 'class' => 'text-white', 'bare' => true])->render(); ?>

                    </span>
                    <span class="text-xs text-gray-600 dark:text-gray-300"><?php echo e(ucfirst($p)); ?></span>
                </button> 
...
(function () {
    ...
    const ICONS = <?php echo json_encode($iconHtml, 15, 512) ?>;
    const BG_CLASSES = <?php echo json_encode($bgClasses, 15, 512) ?>;
    const ALL_KEYS = <?php echo json_encode($platforms, 15, 512) ?>;
...
</script><?php /**PATH D:\Year III\SemesterII\WCT\ShopExpress\resources\views/seller/settings/partials/social-links.blade.php ENDPATH**/ ?>